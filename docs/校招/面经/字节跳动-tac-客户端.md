...
>   飞书妙记：https://swpu.feishu.cn/minutes/obcn5xo65bm9vq581sedn854

1.   自我介绍
2.   Linux 中你写了程序，它肯定以进程的形式运行嘛。操作系统是怎么来管理这些物理内存、虚拟内存，还有包括各个进程的内存，它是怎么一般都是你了解多少，你知道多少就说多少呗。
3.   这个底层的东西你大概知道吗？比如说像你刚才说那操作系统就是这些进程的虚拟内存和真正的物理内存，它之间是大概怎么个映射，怎么个管理？
4.   那我问一个问题，比如说那个进程在申请内存的时候，比如说申要申请很一大块内存，但是可能那个随着。那个之前的别的进程的销毁等等，可能操作系统的那个内存可能是都是各种小的块，那它这个比如说这个大的申请的时候可能没有单独的小的块能满足它需求，但是可能做一些合并就能满足它需求。这个操作系统一般是它会通过什么逻辑？来来来来去来去做合并还是什么？这个你大概清楚吗？
5.   你刚才提到那个，那进程的内存分配，那您说一下那个比如静态存储区，然后全局变量，嗯，栈和堆这些东西它是怎么对？就是你，你描述一下什么情况下会往这三个地方去放什么？什么场景会往这三个场景去放吧
6.   你刚刚提到就是说用 malloc 或者 new 去创建内存，可能是在堆空间，然后需要程序员自己去管理。嗯，那有什么方法可以防止内存的泄露？
7.   你怎么排查一个进程有没有内存泄露？
8.   比如说你有之前有经验定位到一些有内存泄露的地方吗？比如说一般都是什么常见的错误会引起，容易引起内存泄露？
9.   你的代码里面有 new 的地方，但是程序退出的时候没有 delete 的话，如果这个进程退出了，你觉得那个内存会一直挂着那吗？
