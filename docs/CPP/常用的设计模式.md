...
private:
    ...
    static Singleton instance; // 静态成员，程序启动时构造
...
-   实现简单
-   程序启动时由静态初始化保证线程安全（C++11 及以后）

**缺点**：

-   程序一启动就创建，不管是否使用
-   如果初始化依赖外部资源，可能顺序不确定（静态初始化顺序问题）
...
C++11 规定**函数内的静态局部变量**在第一次初始化时是线程安全的，所以可以这样写：
...
| 模式                           | 初始化时机   | 线程安全性 | 资源占用 | 推荐度                 |
| ------------------------------ | ------------ | ---------- | -------- | ---------------------- |
| 饿汉模式                       | 程序启动时   | 天然安全   | 可能浪费 | 中（适合开销小的对象） |
| 懒汉模式（传统）               | 第一次访问时 | 需手动加锁 | 节省资源 | 低（写法复杂，易出错） |
| 懒汉模式（C++11 静态局部变量） | 第一次访问时 | 自动安全   | 节省资源 | 高（推荐）             |

:bulb: **建议**

在现代 C++（C++11 及以后）中，**首选懒汉模式 + 静态局部变量**，因为它：
