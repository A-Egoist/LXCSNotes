...
int main() {
    ...
    double result = 0;
    std::thread root_thread(transform_reduce<std::vector<double>::iterator, decltype(transform), decltype(reduce), double>, numbers.begin(), numbers.end(), transform, reduce, std::ref(result));
    root_thread.join();
